...
Підготувати клієнт для користувача
  ...
  Run Keyword If  '${username}' != 'aukzionua_Viewer'  Run Keywords
  ...  Login  ${username}
...
Створити тендер
  ...
  Wait Until Page Contains Element  xpath=//a[@href="http://test.neb.org.ua/tenders"]  10
  Click Element  xpath=//a[@href="http://test.neb.org.ua/tenders"]
  Click Element  xpath=//a[@href="http://test.neb.org.ua/tenders/index"]
  Click Element  xpath=//a[contains(@href,"http://test.neb.org.ua/buyer/tender/create")]
...
Завантажити ілюстрацію
  ...
  aukzionua.Завантажити документ   ${username}  ${filepath}  ${tender_uaid}  True
...
Пошук тендера по ідентифікатору
  ...
  Switch browser  ${username}
  Go To  http://test.neb.org.ua/tenders/index
  ...
  ...  AND  Wait Until Element Is Visible  id=tenderssearch-tender_cbd_id
...
Перейти на сторінку з інформацією про тендер
  ...
  Input text  id=tenderssearch-tender_cbd_id  ${tender_uaid}
  Click Element  xpath=//button[@data-test-id="search"]
  Click Element  xpath=//*[contains(text(),'${tender_uaid}') and contains('${tender_uaid}', normalize-space(text()))]/ancestor::div[@class="search-result"]/descendant::a
...
Задати запитання на тендер
  ...
  aukzionua.Задати питання  ${username}  ${tender_uaid}  ${question}

Задати запитання на предмет
  [Arguments]  ${username}  ${tender_uaid}  ${item_id}  ${question}
  aukzionua.Задати питання  ${username}  ${tender_uaid}  ${question}  ${item_id}
...
Отримати документ
  ...
  aukzionua_download_file   ${url}  ${file_name}  ${OUTPUT_DIR}
  [return]  ${file_name}
...
Отримати інформацію із документа по індексу
  ...
  ${doc_value}=  convert_string_from_dict_aukzionua  ${doc_value}
  [return]  ${doc_value}
...
Подати цінову пропозицію
  ...
  aukzionua.Пошук тендера по ідентифікатору   ${username}  ${tender_uaid}
  Run Keyword And Ignore Error  aukzionua.Скасувати цінову пропозицію  ${username}  ${tender_uaid}
...
Опублікувати Пропозицію
  ...
  ...  Go To  http://test.neb.org.ua/bids/send/${url.split('?')[0].split('/')[-1]}?token=465
  ...  ELSE  Go To  http://test.neb.org.ua/bids/decline/${url.split('?')[0].split('/')[-1]}
  Go To  ${url}
...
Змінити цінову пропозицію
  ...
  ${file_path}=  get_upload_file_path
  aukzionua.Пошук тендера по ідентифікатору   ${username}  ${tender_uaid}
  aukzionua.Скасувати цінову пропозицію  ${username}  ${tender_uaid}
  Wait Until Element Is Visible   xpath=//input[contains(@name, '[value][amount]')]
...
Завантажити документ в ставку
  ...
  ...  aukzionua.Отримати інформацію із пропозиції  ${username}  ${tender_uaid}  ${EMPTY}
  ...  AND  aukzionua.Скасувати цінову пропозицію  ${username}  ${tender_uaid}
...
Завантажити фінансову ліцензію
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}
  aukzionua.Завантажити документ в ставку  ${username}  ${filepath}  ${tender_uaid}

Змінити документ в ставці
  [Arguments]  ${username}  ${tender_uaid}  ${path}  ${docid}
  aukzionua.Завантажити документ в ставку  ${username}  ${path}  ${tender_uaid}
...
Conv And Select From List By Value
  ...
  ${smth_to_select}=  convert_string_from_dict_aukzionua  ${smth_to_select}
