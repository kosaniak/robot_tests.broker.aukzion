...
def convert_datetime_to_aukzionua_format(isodate):
...
def convert_string_from_dict_aukzionua(string):
    return {
        u"грн": u"UAH",
        u"True": u"1",
        u"False": u"0",
...
def adapt_view_data(value, field_name):
    if field_name == 'value.amount':
        ...
        value = int(value)
    return convert_string_from_dict_aukzionua(value)
...
def get_upload_file_path():
    return os.path.join(os.getcwd(), 'src', 'robot_tests.broker.aukzionua', 'testFileForUpload.txt')


def aukzionua_download_file(url, file_name, output_dir):
